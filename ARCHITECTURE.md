...
**Chrome Web Store (current approach):** To resolve the self-hosted loading issue, the extension was submitted to the Chrome Web Store (ID: `ojhmfklcaknmocfiibdeclhahffofgan`). Web Store extensions are treated as first-class citizens by ChromeOS kiosk mode and install reliably via the Admin Console's kiosk app/extension configuration. **Version 3.4.0 is currently pending Google's review.** Once approved, v3.5.3 will be uploaded as an update. The `EXTENSION_ID` in `application.js` currently still references the self-hosted ID and will need to be updated to the Web Store ID once the extension is published and confirmed working on production kiosk devices.
...
1. **Chrome Web Store review** — Extension v3.4.0 pending. Once approved, upload v3.5.3 as update.
